...
const ControlPanel: React.FC<ControlPanelProps> = ({ state, setState, onDownload }) => {
  ...
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    { role: 'model', text: 'Hello! I can help you with gear ratios, material choices, or explaining involute geometry.' }
  ]);
  ...
  const handleAiSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, { role: 'model', text: 'Error: Could not connect to AI. Check API Key.', isError: true }]);
    } finally {
      ...
      {activeTab === 'settings' ? (
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div className="space-y-3">
                <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">System Parameters</h3>
                
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Module (Size)</label>
                        <input 
                    ...
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Pressure Angle (°)</label>
                        <input 
                            type="number"
                 ...
                 <div className="flex items-center gap-2">
                     ...
                     />
                     <label htmlFor="lockRatio" className="text-sm text-slate-300 cursor-pointer select-none">Lock Ratio when resizing Gear 1</label>
                 </div>
            </div>
            ...
            <div className="space-y-3 border-t border-slate-700 pt-4">
                <div className="flex justify-between items-center">
                     <h3 className="text-xs font-semibold text-brand-500 uppercase tracking-wider">Driver Gear (1)</h3>
                     <button onClick={() => onDownload(1)} className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-white" title="Download SVG">
                        <Download className="w-4 h-4" />
                     </button>
                </div>
                <div>
                    <label className="block text-xs text-slate-400 mb-1">Teeth Count: {state.gear1.toothCount}</label>
                    <input 
            ...
            <div className="space-y-3 border-t border-slate-700 pt-4">
                <div className="flex justify-between items-center">
                     <h3 className="text-xs font-semibold text-rose-500 uppercase tracking-wider">Driven Gear (2)</h3>
                     <button onClick={() => onDownload(2)} className="p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-white" title="Download SVG">
                        <Download className="w-4 h-4" />
                     </button>
                </div>
                <div>
                    <label className="block text-xs text-slate-400 mb-1">Teeth Count: {state.gear2.toothCount}</label>
                    <input 
                        type="range" min="4" max="100"
            ...
            <div className="border-t border-slate-700 pt-4">
                ...
                >
                    {state.isPlaying ? <><Pause className="w-4 h-4" /> Stop Simulation</> : <><Play className="w-4 h-4" /> Simulate</>}
                </button>
                <div className="mt-4">
                     <label className="block text-xs text-slate-400 mb-1">Speed (RPM)</label>
                     <input 
                 ...
                 {process.env.API_KEY ? (
                     <form onSubmit={handleAiSubmit} className="flex gap-2">
                        <input 
                            ...
                            placeholder="Ask about gear ratios..."
                    ...
                    <div className="flex items-center gap-2 text-yellow-500 text-xs p-2 bg-yellow-900/20 border border-yellow-900 rounded">
                        ...
                        <span>API Key missing in env. Assistant disabled.</span>
                    </div>
                 )}
            </div>
        </div>
